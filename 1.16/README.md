...
Example:
    ```
    git clone git@gitlab.cee.redhat.com:sturivny/nginx-container.git
    cd nginx-container/1.16
    s2i build . --context-dir=test/test-app/ centos/nginx-116-centos7 nginx-naxsi-app
    docker run -p 8080:8080 nginx-naxsi-app
    ```
